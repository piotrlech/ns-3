...
class AqmQueueNewtonStepTest;  // Forward declaration for unit test
class AqmQueueControlLawTest;  // Forward declaration for unit test

namespace ns3 {

static const int  AQM_SHIFT = 10;
static const int DEFAULT_AQM_LIMIT = 1000;
 ...
 * \ingroup queue
 *
 * \brief A Aqm packet queue
 */

class AqmQueue : public Queue
{
public:
  static TypeId GetTypeId (void);

  /**
   * \brief AqmQueue Constructor
   *
   * Creates a Aqm queue
   */
  AqmQueue ();
   ...
   */
  void SetMode (AqmQueue::QueueMode mode);
   ...
   */
  AqmQueue::QueueMode  GetMode (void);
  ...
  /**
   * \brief Get the number of packets dropped according to Aqm algorithm
   *
   * \returns The number of dropped packets
   */
  uint32_t GetDropCount (void);

  static Time GetLowDelay (void);
  static Time GetHighDelay (void);
...
private:
  friend class::AqmQueueNewtonStepTest;  // Test code
  friend class::AqmQueueControlLawTest;  // Test code
  /**
   ...
   * Aqm control law is t + m_interval/sqrt(m_count).
  ...
  bool AqmTimeAfter (uint32_t a, uint32_t b);
  bool AqmTimeAfterEq (uint32_t a, uint32_t b);
  bool AqmTimeBefore (uint32_t a, uint32_t b);
  bool AqmTimeBeforeEq (uint32_t a, uint32_t b);

  /**
   * returned unsigned 32-bit integer representation of the input Time object
   * units are microseconds
   */
  uint32_t Time2Aqm (Time t);
  ...
  Time m_lowDelay;                        //!< Low queue delay
  Time m_highDelay;                       //!< High queue delay
  Time static sm_lowDelay;
  Time static sm_highDelay;
  TracedValue<uint32_t> m_count;          //!< Number of packets dropped since entering drop state
  TracedValue<uint32_t> m_dropCount;      //!< Number of dropped packets according Aqm algorithm
  ...
  uint32_t randv;						  //!< Random variable
};
